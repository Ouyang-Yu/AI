...
from PyQt5.QtWidgets import QMainWindow,QFileDialog,QMessageBox,QInputDialog,QApplication
...
class mywindow(Ui_MainWindow,QMainWindow):
    ...
    def __init__(self):
     ...
     #   self.label.setPixmap(QPixmap("1.jpg"))
        ...
        # 更新用户信号槽
        self.actionupdateuser.triggered.connect(self.update_user)
        #关联时间/日期的定时器信号与槽函数
    ...
    def on_actionopen(self):
        list = self.getlist()
        self.group_id, ret = QInputDialog.getText(self, "请输入所要签到的用户组", "已存在的用户组如下\n" + str(list['result']['group_id_list']))
        if self.group_id == '':
        ...
        self.detect_data_signal.connect(self.detectThread.get_base64)#获得返回的人脸信息
        self.detectThread.transmit_data.connect(self.get_detectdata)
    ...
    def get_accesstoken(self):
        ...
        host= 'https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=WhKHcoHxI15WBM1SfHD5Qegq&client_secret=cXIUTDOP2qhBOknXZq4IgskY5LSY0VpT'
        #发送网络请求   requests网络库
